...
├── skills/
│   ├── develop/                   ← two-lane workflow (Lane 2 = 8 phases)
...
- **Gradle wrapper:** scaffolds ship with a pinned Gradle 9.2.0 (GA) wrapper (`gradlew`, `gradlew.bat`, `gradle/wrapper/`). No system `gradle` install required — `./gradlew build` works directly after `/modsmith:init`. You do need an installed JDK 21+ for Gradle itself to run on (the Loom/MDG plugin jars require it); the scaffolded `gradle/gradle-daemon-jvm.properties` auto-selects a matching installed JDK even when `JAVA_HOME` points at an older one.
